...
def chunk_list(sample_list, chunk_size):
    return [
        sample_list[i: i + chunk_size]  # noqa: E203
...
def train_model(model, train_lables, batch_size, train_images, train_path, val_dataset, test_dataset, epochs,
                callbacks):
    train_data_loader = ArcFaceDataLoader(
        base_path=train_path,
        labels=train_lables,
        filenames=train_images,
        target_width=300,
        target_height=300,
    ...
    model.fit(train_dataset, epochs=epochs, validation_data=val_dataset, callbacks=callbacks, verbose=2)
...
if __name__ == "__main__":
    ...
    distribution_size = len(args.hosts)
    ...
    if distribution_size > 1:
        index = sorted(args.hosts).index(args.current_host)
        train_image_paths = chunk_list(train_image_paths, len(train_image_paths) // distribution_size + 1)[
            index
        ]
        train_image_labels = chunk_list(
            train_image_labels, len(train_image_labels) // distribution_size + 1
        )[index]
    ...
    train_model(model,
                ...
                train_lables=train_image_labels,
                train_path=train_path,
                val_dataset=val_dataset,
                test_dataset=test_dataset,
                batch_size=batch_size,
                callbacks=[tensorboard_callback,
                           lr_callback, checkpoint])
